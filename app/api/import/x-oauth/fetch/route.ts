...
export async function POST(req: NextRequest) {
  ...
  if (!userId?.value) {
    return NextResponse.json({ error: 'X user ID not found. Please reconnect your account.' }, { status: 401 })
  }
  ...
  for (let page = 0; page < maxPages; page++) {
    ...
    const res = await fetch(`https://api.x.com/2/users/${userId.value}/bookmarks?${params}`, {
