...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  let preflightClient: AIClient | null = null
  if (isTextOnlyProvider(provider)) {
    try {
      preflightClient = await resolveAIClient({ dbKey: dbApiKey })
    } catch (err) {
      const message = err instanceof Error
        ? err.message
        : 'OpenAI-compatible provider requires a base URL and direct HTTP client configuration.'
  ...
  void (async () => {
    ...
    try {
      let client: AIClient | null = preflightClient
      if (!client) {
        try {
        ...
        const visionEnabled = !isTextOnlyProvider(provider)
